...
typedef std::vector<TGeoVolume*> entity_collection_t;
typedef std::map<int,TGeoVolume*> entity_id_map_t;
typedef std::map<std::string,TGeoVolume*> entity_name_map_t;

/**
 * Contains functions for the creation and retreval of surface equivalent volumes
 * from the root geometry manager
 */
class SurfaceManager {
  public:
   SurfaceManager(TGeoManager *);
  ~SurfaceManager();
  makeSurface();
  private:
  TGeoManager *geo;
  entity_id_map_t surface_list;
  entity_name_map_t surface_name;
};
...
std::string mcnp2root_version(bool full = true);
...
int main(int argc, char* argv[]){
  ...
  ProgOptions po("mcnp2root " + mcnp2cad_version(false) +  ": An MCNP geometry to Root file converter");
  po.setVersion( mcnp2root_version() );
  ...
  TGeoManager *geo = new TGeoManager("world","mcnp2root");

  GeometryContext context( geo, deck );
 ...
 */
std::string mcnp2root_version( bool full ){
  std::stringstream str;
  str << (full ? "mcnp2root version " : "")
      << MCNP2ROOT_VERSION_MAJOR << "." 
      << MCNP2ROOT_VERSION_MINOR << "." 
      << MCNP2ROOT_VERSION_REV;
  if(full)
